(function(exports) {
	...
	Renderer.prototype = {
		...
		renderAll: function() {
			...
			for (var j=Y1; j<=Y2; j++)
				for (var i=X1; i<=X2;i++) {
					var planeX = Math.floor(i/w)*w, planeY = Math.floor(j/h)*h;
					var p = this.game.getObj(i - planeX, j - planeY);
					if (p) {
						...
						var cx = p.dx + planeX*TILE, cy = p.dy + planeY*TILE;
						if (p.type.renderShiftX) cy += p.type.renderShiftX;
						if (p.type.renderShiftY) cy += p.type.renderShiftY;
						var renderHeight = p.type.renderWidth * frameHeight / p.type.frameWidth;
						var sH = frameHeight, dH = renderHeight * p.size;
						var sy = p.sy + (p.color||0) * p.type.framesY;
						if (p.mirror) {
							context.save();
							context.translate(cx, cy);
							context.scale(-1, 1);
							context.drawImage(p.type.image, 
								p.sx*sW, sy*sH, // skin coord
								sW, sH, // skin crop
