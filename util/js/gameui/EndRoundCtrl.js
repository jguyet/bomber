bombermine.controller('EndRoundCtrl', function($location, $scope, $rootScope, $http, localize, Social, Keys, Game) {
...
$scope.scoreboardDeathClose = function() {
  ...
  var $board = $('#scoreboard-end-round');
  var $filter = $('#end-round-name-filter');
  var $wrap = $board.find('#end-round-list-wrap');
  var $scrollbar = $wrap.find('#end-round-list-scrollbar');
  var $thumb = $scrollbar.find('#end-round-list-thumb');

  var filtered;

  var sortInterval, redrawDelay, nicknameFilterTimeout;
  ...
  $filter.on('focusin', function () {
    ...
    if (nicknameFilterTimeout || $(this).val().length < 2)
      return;
